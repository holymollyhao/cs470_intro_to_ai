...
class IMDBDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file='../dataset/ichar/minmax_scaling_all.csv',
        ...
        if domains[0].startswith('train'):
            self.data = imdb_dataset(directory=self.file_path, train=True)
        elif domains[0].startswith('test'):
            self.data = imdb_dataset(directory=self.file_path, test=True)
    ...
    def preprocessing(self, raw_text=False):
        ...
        for d_dic in self.data:
            if raw_text:
                text = d_dic['text']
            else:
                text = tokenize(self.tokenizer, d_dic['text'])
            label = self.class_dict[d_dic['sentiment']] # convert string to class index
