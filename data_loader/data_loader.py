...
from .VisDADataset import VisDADataset
from .CMNISTDataset import CMNISTDataset
from .RMNISTDataset import RMNISTDataset
from .DomainNetDataset import DomainNetDataset
from .TerraIncognitaDataset import TerraIncognitaDataset
        ...
        if len(datasets):
            ...
            if sum([len(dataset) for dataset in datasets]) > 0:  # at least one dataset has data
                ...
                data_loader = DataLoader(torch.utils.data.ConcatDataset(datasets), batch_size=batch_size,
                                         shuffle=shuffle, drop_last=drop_last, pin_memory=False, num_workers=num_workers)
        return data_loader
    else:
        data_loaders = []
...
def domain_data_loader(dataset, domains, file_path, batch_size, train_max_rows=np.inf, valid_max_rows=np.inf,
                       ...
                       num_source=9999, tokenizer=None):
    entire_datasets = []
    train_datasets = []
    ...
    if dataset in ['harth']:
        ...
        if not loaded_data:
            loaded_data = TinyImageNetDataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        loaded_data = load_cache(dataset, processed_domains, file_path, transform=transform)
        if not loaded_data:
            loaded_data = SVHNDataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path)
    elif dataset in ['visda']:
        ...
        loaded_data = load_cache(dataset, processed_domains, file_path, transform=transform)
        if not loaded_data:
            loaded_data = VisDADataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path)
        ...
        if not loaded_data:
            loaded_data = CMNISTDataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        if not loaded_data:
            loaded_data = RMNISTDataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)

    elif dataset in ['terra_incognita']:
        ...
        if not loaded_data:
            loaded_data = TerraIncognitaDataset(file=file_path, domains=cond, max_source=num_source,
                                                transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        if not loaded_data:
            loaded_data = DomainNetDataset(file=file_path, domains=cond, max_source=num_source, transform=transform)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        if not loaded_data:
            loaded_data = MNISTDataset(file=file_path, domains=cond, max_source=num_source)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)
        ...
        if not loaded_data:
            loaded_data = IMDBDataset(file=file_path, domains=cond, max_source=num_source, tokenizer=tokenizer)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)

    elif dataset in ['sst-2']:
        cond = processed_domains
        ...
        if not loaded_data:
            loaded_data = SST2Dataset(file=file_path, domains=cond, max_source=num_source, tokenizer=tokenizer)
            save_cache(loaded_data, dataset, processed_domains, file_path, transform=transform)

    elif dataset in ['finefood']:
        cond = processed_domains
